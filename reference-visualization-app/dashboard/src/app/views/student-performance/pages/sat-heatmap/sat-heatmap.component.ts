...
@Component({
  selector: 'app-sat-heatmap',
  templateUrl: './sat-heatmap.component.html',
  styleUrls: ['./sat-heatmap.component.scss']
})
export class SatHeatmapComponent implements OnInit {
  ...
  fetchFilters(metaData) {
    ...
    this.examDates = [{ exam_date: "all" }, ...this.examDates.filter(item => item)];
    this.grades = [{ grade: "all" }, ...this.grades.filter(item => item)];
  }
