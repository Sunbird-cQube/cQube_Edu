...
check_az_storage_connection_string(){
    ...
    az storage container list  --connection-string "$azure_storage_connection_string" > /dev/null 2>&1
...
check_az_container(){
  ...
  export AZURE_STORAGE_CONNECTION_STRING="$1"
  containerstatus=$(az storage container exists --connection-string "$azure_storage_connection_string" --name $2 | grep -c false )
...
declare -a arr=("azure_input_container" "azure_output_container" "azure_emission_container" "azure_account_name" "azure_account_key" "azure_storage_connection_string")
