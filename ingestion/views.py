    ...
    try:
        ...
        if wdf["score"].sum() == 100:
            if list(df.columns) == list(wdf.columns):
                if len(df[['infrastructure_name', 'infrastructure_category']].merge(
                        wdf[['infrastructure_name', 'infrastructure_category']]).drop_duplicates()) == len(df):
                    return True
                else:
                    abort(400,
                          f'Bad data in file, please validate the data in infrastructure_name and '
                          f'infrastructure_category')
            else:
...
def create_onpremise_url(destination_folder):
    try:
        url = {'url': f"{destination_folder}"}
    except ClientError as e:
        logging.error(e)
        return None
    return url
