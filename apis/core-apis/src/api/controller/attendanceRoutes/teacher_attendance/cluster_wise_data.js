...
router.post('/clusterWise', auth.authController, async (req, res) => {
    try {
        ...
        res.status(200).send({ clusterData: clusterData, teacherCount: jsonData.allClustersFooter.teachers, schoolCount: jsonData.allClustersFooter.schools, dateRange: dateRange, fileMetaData });
    } catch (e) {
...
router.post('/clusterPerBlock', auth.authController, async (req, res) => {
    try {
        ...
        res.status(200).send({ clusterDetails: clusterData, teacherCount: jsonData.footer[blockId].teachers, schoolCount: jsonData.footer[blockId].schools, dateRange: dateRange, fileMetaData });
    } catch (e) {
        logger.error(`Error :: ${e}`)
        res.status(500).json({ errMessage: "Internal error. Please try again!!" });
    }
